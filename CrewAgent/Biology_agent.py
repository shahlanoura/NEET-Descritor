...
def generate_biology_questions(text_content, num_questions):
    try:
...
*Source Material*:
1. Biology textbook units (provided).
...
"""
        ...
        result_str = crew_instance.kickoff(inputs={'prompt': prompt, 'topic': 'Biology', 'num_questions': num_questions})
        logging.debug("Generated result string: %s", result_str)
        return result_str
    except Exception as e:
        logging.error("Error in generate_Biology_questions: %s", e)
        return None
